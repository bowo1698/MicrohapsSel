...
all_results <- foreach(
  fold = 1:config$k_folds,
  .packages = c("tidyverse", "sommer", "coda", "masbayes"),
  .errorhandling = "pass",
  .verbose = FALSE
) %dopar% {
  ...
  tryCatch(
    {
      ...
      save_fold_results(
        fold, gblup_res, bayesR_res, bayesA_res, matrices, config$output_dir
      )

      save_gebv_with_regression(
        fold, gblup_res, bayesR_res, bayesA_res, split, config$output_dir
      )
      ...
      result <- aggregate_fold_results(
        gblup_res, bayesR_res, bayesA_res, fold
      )
