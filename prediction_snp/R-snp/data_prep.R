...
setup_cv_folds <- function(phenotype_ref, config) {
  ...
  if(family_check$n_sires > 1 && family_check$median_offspring >= 5) {
    method <- "stratified"
    folds <- caret::createFolds(y = phenotype_ref$father_id, 
                                k = config$k_folds, 
                                list = FALSE, returnTrain = FALSE)
    reason <- NULL
    sires_per_fold <- phenotype_ref %>%
      mutate(fold = folds) %>%
      group_by(fold) %>%
      summarise(n_sires = n_distinct(father_id)) %>%
      pull(n_sires)
    avg_folds_per_sire <- mean(table(folds))
  } else {
    method <- "random"
    folds <- sample(rep(1:config$k_folds, length.out = nrow(phenotype_ref)))
    reason <- paste0("Insufficient family structure (", family_check$n_sires, 
                     " sires, median ", family_check$median_offspring, " offspring)")
    sires_per_fold <- NULL
