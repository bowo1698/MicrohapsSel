...
run_bayesA <- function(matrices, split, gblup_varcomp, config, fold = 0) {
  ...
  cat("BayesA Settings:\n")
  cat("ν =", nu, "| σ²_g =", round(sigma2_g, 6), "\n")
  ...
  pred_test <- W_test %*% beta_A_hat + mu_hat
  ...
  accuracy_test_pheno <- if (has_test) cor(pred_test, pheno_test$phenotype, use="complete.obs") else NA
  accuracy_test_tbv   <- if (has_test) cor(pred_test, pheno_test$tbv, use="complete.obs") else NA

  h2_test_bayesa     <- if (has_test) var(as.vector(pred_test)) / (var(as.vector(pred_test)) + sigma2_e_A_hat) else NA
  accuracy_test_gebv <- if (has_test && !is.na(h2_test_bayesa)) accuracy_test_pheno / sqrt(h2_test_bayesa) else NA
  ...
  b_test_pheno <- if (has_test) calculate_regression_slope(pred_test, pheno_test$phenotype) else NA
  b_test_tbv   <- if (has_test) calculate_regression_slope(pred_test, pheno_test$tbv) else NA
  ...
  rmse_test_pheno <- if (has_test) sqrt(mean((pred_test - pheno_test$phenotype)^2, na.rm=TRUE)) else NA
  rmse_test_tbv   <- if (has_test) sqrt(mean((pred_test - pheno_test$tbv)^2, na.rm=TRUE)) else NA
  ...
  r2_test_pheno <- if (has_test) cor(pred_test, pheno_test$phenotype, use="complete.obs")^2 else NA
  r2_test_tbv   <- if (has_test) cor(pred_test, pheno_test$tbv, use="complete.obs")^2 else NA
