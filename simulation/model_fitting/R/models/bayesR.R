...
run_bayesR <- function(matrices, split, gblup_varcomp, config, fold = 0) {
  ...
  h2_test_bayesr  <- if (has_test) var(as.vector(pred_test)) / (var(as.vector(pred_test)) + sigma2_e_hat) else NA
  accuracy_test_gebv <- if (has_test && !is.na(h2_test_bayesr)) accuracy_test_pheno / sqrt(h2_test_bayesr) else NA
