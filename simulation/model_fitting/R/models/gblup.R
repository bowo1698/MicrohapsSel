...
run_gblup <- function(matrices, split, config) {
  ...
  accuracy_test_pheno <- if (has_test) cor(pred_test, pheno_test$y) else NA
  accuracy_test_tbv   <- if (has_test) cor(pred_test, pheno_test$tbv) else NA
  accuracy_test_gebv  <- if (has_test && !is.na(h2_test_gblup)) accuracy_test_pheno / sqrt(h2_test_gblup) else NA
  ...
  b_test_pheno <- if (has_test) calculate_regression_slope(pred_test, pheno_test$y) else NA
  ...
  rmse_test_pheno <- if (has_test) sqrt(mean((pred_test - pheno_test$y)^2, na.rm=TRUE)) else NA
  ...
  r2_test_pheno <- if (has_test) cor(pred_test, pheno_test$y)^2 else NA
  r2_test_tbv   <- if (has_test) cor(pred_test, pheno_test$tbv)^2 else NA
