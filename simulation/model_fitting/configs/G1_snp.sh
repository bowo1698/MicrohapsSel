#!/bin/bash
#SBATCH --job-name=fit_cv_G1_snp_test
#SBATCH --array=1 #1-20
...
#SBATCH -o logs/fit_cv_G1_snp_test_%A_%a.out
#SBATCH -e logs/fit_cv_G1_snp_test_%A_%a.err
...
echo "=== ITER=${ITER} SNP Started: $(date) ==="

TMPCONFIG=$(mktemp /tmp/config_snp_cv_XXXXXX.R)
cat > $TMPCONFIG << EOF
config <- list(
  base_dir            = "${MODEL}",
...
Rscript ${MODEL}/main-snp-fit-eval.R
rm -f $TMPCONFIG

echo "=== ITER=${ITER} SNP Completed: $(date) ==="