#!/bin/bash
#SBATCH --job-name=pred_mh_G2
#SBATCH --array=1-40 #1-40
...
#SBATCH -o logs/pred_mh_G2_%A_%a.out
#SBATCH -e logs/pred_mh_G2_%A_%a.err
...
G2_DIR="${TMPDIR}/iter${ITERPAD}_${SOURCE_COMBO}_mh"

echo "=== TASK=${TASK_ID} | ITER=${ITER} | SOURCE=${SOURCE_COMBO} | MH pred started: $(date) ==="
...
if [ -f "${OUT_DIR}/mh/S6_unequal/bayesR/predict_cv.rds" ]; then
  echo "SKIP — already done: ${OUT_DIR}/mh"
  exit 0
fi
...
config <- list(
  base_dir      = "${MODEL}",
  data_dir      = "${G2_DIR}",
  output_dir    = "${OUT_DIR}/mh",
  block_dir     = "mh_genotypes",
  pedigree_file = "pedigree_G2.csv",
...
echo "=== TASK=${TASK_ID} | ITER=${ITER} | SOURCE=${SOURCE_COMBO} | MH pred done: $(date) ==="